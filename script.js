...
function data(result, word) {
    if (result.title) {
        ...
        let definitions = result[0].meanings[0].definitions[0],
            phontetics = `${result[0].meanings[0].partOfSpeech} /${result[0].phonetics[0]?.text || "No transcription"}/`;
        ...
        if (result[0].phonetics[0]?.audio) {
            ...
            volume.style.display = "inline"; // Show the volume icon if audio exists
        } else {
            audio = null;
